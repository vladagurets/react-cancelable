...
export default function useCancelableReq(_a) {
    ...
    function processRes(response, isMounted) {
        if (response.ok) {
            ...
            response[resHandler]()
                .then(function (data) { return rejectOrCb(onSetRes, { isMounted: isMounted, data: data }); });
        }
        else {
            var resHandler = getResParser(response.headers.get('Content-Type') || 'default');
            response[resHandler]()
                .then(function (data) { return rejectOrCb(onSetError, { isMounted: isMounted, data: data }); });
        }
    }
    function rejectOrCb(cb, opts) {
        var isMounted = opts.isMounted, data = opts.data;
        if (isMounted) {
            cb(data, isMounted);
        }
        else {
    ...
    useEffect(function () {
        ...
        fetch(src, __assign(__assign({}, fetchParams), { signal: controller.signal }))
            .then(function (response) { return rejectOrCb(processRes, { isMounted: isMounted, data: response }); })
            .catch(function (error) { return rejectOrCb(onSetError, { isMounted: isMounted, data: error }); });
        return function () {
            isMounted = false;
